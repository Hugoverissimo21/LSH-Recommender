...
saved_metrics_file = "resultsv3.csv"
...
train = train.join(user_index, on="userId").repartition(4)
...
item_user = train.groupBy("movieId") \
    .agg(collect_list(struct("user_idx", "rating")).alias("user_ratings")).repartition(4)
...
neighbors = lsh_model.approxSimilarityJoin(
    ...
    threshold=threshold,
    distCol="distance"
).filter(col("datasetA.movieId") < col("datasetB.movieId")).repartition(4)
...
reverse = neighbors_cosine.selectExpr("movie_j as movie_i", "movie_i as movie_j", "cosine_sim").repartition(4)
similarities = neighbors_cosine \
    .union(reverse) \
    .withColumn("rank", row_number().over(Window.partitionBy("movie_i").orderBy(col("cosine_sim").desc()))) \
    .filter(col("rank") <= 100) \
    .drop("rank").repartition(4)

# filter the similarities to only include movies in the test set
test_movies = test.select("movieId").distinct().repartition(4)
similarities = similarities.join(test_movies, similarities.movie_i == test_movies.movieId).repartition(4)
